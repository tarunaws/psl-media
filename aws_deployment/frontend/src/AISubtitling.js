...
export default function AISubtitling() {
  ...
  const selectedS3Meta = useMemo(() => {
    if (!selectedS3Key) return null;
    return rawVideoItems.find((item) => item.key === selectedS3Key) || null;
  }, [rawVideoItems, selectedS3Key]);
  ...
  const browseS3 = useCallback(async (prefix) => {
    ...
    try {
      ...
      setS3Folders([]);
      setRawVideoItems([]);
      const backendMessage = fetchError.response?.data?.error || fetchError.message;
      setError(`Failed to browse S3: ${backendMessage}`);
    } finally {
  ...
  const startProcessing = async () => {
    const hasUploadSelection = Boolean(selectedFile);
    const hasS3Selection = Boolean(selectedS3Key);
    const canStart = inputMode === 'upload' ? hasUploadSelection : hasS3Selection;
    if (!canStart || processing) return;

    const reuseExisting = Boolean(currentFileId && phase === 'complete' && inputMode === 'upload');
  ...
  setStreams({});
    ...
    try {
      ...
      if (inputMode === 'upload') {
        ...
        setProgressMessage('Starting download from S3...');
  ...
  return (
    <Page>
      ...
      {inputMode === 'upload' && selectedFile && (
        <VideoPreview>
          ...
          <div style={{ marginTop: '1rem' }}>
            <Button onClick={startProcessing} disabled={processing || !selectedFile}>
      ...
      {inputMode === 's3' && selectedS3Key && (
        <VideoPreview>
          <VideoInfo>
            <VideoName>{selectedS3Key}</VideoName>
            <VideoSize>
              {selectedS3Meta?.size ? formatFileSize(selectedS3Meta.size) : '—'}
            </VideoSize>
          </VideoInfo>
          <FileMeta>
            <FileMetaRow>
            ...
            <FileMetaRow>
              <FileLabel>Last modified</FileLabel>
              <span>
                {selectedS3Meta?.last_modified
                  ? new Date(selectedS3Meta.last_modified).toLocaleString()
                  : '—'}
              </span>
            </FileMetaRow>
          </FileMeta>
          <LanguageControls>
            <ControlGroup>
              <Label htmlFor="source-language-s3">Source language</Label>
              <Select
                id="source-language-s3"
                value={sourceLanguage}
                onChange={(event) => setSourceLanguage(event.target.value)}
                disabled={processing}
              >
          ...
          <div style={{ marginTop: '1rem' }}>
            <Button onClick={startProcessing} disabled={processing || !selectedS3Key}>
              {processing ? 'Processing…' : 'Generate Subtitles'}
            </Button>
            <SecondaryButton
              onClick={removeFile}
            ...
            <VideoName>{formatS3Label(selectedS3Key)}</VideoName>
