...
configure_json_logging('imageCreation')
LOGGER = logging.getLogger('imageCreation')

_image_secret_id = os.environ.get('IMAGE_CREATION_SECRET_ID') or os.environ.get('APP_SECRETS_ID')
if _image_secret_id:
...
_image_sentry_dsn = os.environ.get('IMAGE_SENTRY_DSN') or os.environ.get('SENTRY_DSN')
if sentry_sdk and FlaskIntegration and _image_sentry_dsn:
    sentry_sdk.init(
        dsn=_image_sentry_dsn,
...
DEFAULT_RATE_LIMIT = os.environ.get('DEFAULT_RATE_LIMIT', '120 per minute')
IMAGE_PROMPT_RATE_LIMIT = os.environ.get('IMAGE_PROMPT_RATE_LIMIT', '6 per minute')
CONTACT_RATE_LIMIT = os.environ.get('CONTACT_RATE_LIMIT', '3 per minute')
...
def _wants_html_response() -> bool:
    """Return True when this request should render the HTML UI."""
    if request.is_json:
        return False
    accept = (request.headers.get('Accept') or '').lower()
    if 'application/json' in accept and 'text/html' not in accept:
        return False
    # Default browser form submission: treat as HTML.
    return True

@app.route('/send_prompt', methods=['POST'])
@limiter.limit(IMAGE_PROMPT_RATE_LIMIT)
def send_prompt():
