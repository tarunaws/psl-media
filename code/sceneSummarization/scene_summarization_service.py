...
def _safe_client(service_name: str, region: str | None, *, client_config: Config | None = None):
    try:
        client_kwargs: Dict[str, Any] = {"region_name": region}
        if client_config is not None:
            client_kwargs["config"] = client_config
        return boto3.client(service_name, **client_kwargs)  # type: ignore[call-arg]
...
BEDROCK_CONNECT_TIMEOUT = _parse_timeout("SCENE_SUMMARY_BEDROCK_CONNECT_TIMEOUT", 5)
BEDROCK_READ_TIMEOUT = _parse_timeout("SCENE_SUMMARY_BEDROCK_READ_TIMEOUT", 25)
BEDROCK_GENERATION_TIMEOUT = _parse_timeout("SCENE_SUMMARY_LLM_TIMEOUT_SECONDS", 18)
...
bedrock_runtime = _safe_client("bedrock-runtime", BEDROCK_REGION, client_config=BEDROCK_CLIENT_CONFIG)
polly = _safe_client("polly", POLLY_REGION)
s3 = _safe_client("s3", os.getenv("S3_REGION") or BEDROCK_REGION)


def _service_ready() -> bool:
    return bool(rekognition)
...
def _extract_frames(video_path: str) -> List[str]:
    """Extract representative frames for analysis from across the video timeline."""
    extracted: List[str] = []
    ...
    # leave directory on disk for downstream cleanup if frames exist


def _analyse_media(file_path: str, media_type: str) -> Dict[str, Any]:
...
def _bedrock_summary(metadata: Dict[str, Any]) -> Dict[str, Any]:
...
def _generate_summary(metadata: Dict[str, Any]) -> Dict[str, Any]:
    if bedrock_runtime:
        timeout_seconds = max(5, BEDROCK_GENERATION_TIMEOUT)
        executor = ThreadPoolExecutor(max_workers=1)
        future = executor.submit(_bedrock_summary, metadata)
        try:
            return future.result(timeout=timeout_seconds)
        except FutureTimeoutError:
            future.cancel()
            app.logger.warning(
                "Bedrock summarization exceeded %ss timeout; using heuristic fallback.",
                timeout_seconds,
            )
        ...
        app.logger.info("Bedrock runtime unavailable; using heuristic fallback summary.")
    return _fallback_summary(metadata)


def _synth_audio(file_id: str, ssml: str, voice_id: str) -> str | None:
    if not polly:
        app.logger.warning("Speech synthesis client unavailable; skipping audio render for %s", file_id)
        return None
    try:
    ...
    return jsonify(
        {
            ...
            "services": {
                "vision": bool(rekognition),
                "language": bool(bedrock_runtime),
                "speech": bool(polly),
            },
        }
    )
...
def summarize_scene() -> Any:
    ...
    try:
        structured_metadata = _analyse_media(saved_path, media_type)
